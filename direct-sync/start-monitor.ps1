...
for ($i = 1; $i -le 40; $i++) { Start-Sleep 3; try { $h = (Invoke-WebRequest -Uri "http://localhost:3000/api/health" -UseBasicParsing -TimeoutSec 3).Content | ConvertFrom-Json; if ($h.ok -and $h.mega -and $h.wa) { Show-Notify "WhatsApp Sync Running" "MEGA + WhatsApp connected"; break } } catch {} }
while ($true) { Start-Sleep 600; try { Invoke-WebRequest -Uri "http://localhost:3000/api/health" -UseBasicParsing -TimeoutSec 5 | Out-Null } catch { & $NodePath $Pm2Path restart wa-mega-sync 2>$null } }
...
function notifyWin(title, msg) {
  try {
    const { exec } = require("child_process");
    const ps = 'powershell -Command "Add-Type -AssemblyName System.Windows.Forms; \$n=New-Object System.Windows.Forms.NotifyIcon; \$n.Icon=[System.Drawing.SystemIcons]::Information; \$n.BalloonTipTitle=\''+title+'\'; \$n.BalloonTipText=\''+msg+'\'; \$n.Visible=\$true; \$n.ShowBalloonTip(8000); Start-Sleep 5; \$n.Dispose()"';
    exec(ps, () => {});
  } catch(_) {}
}
...
const deleteGroups=new Set();    // delete on/off per group
...
app.get("/api/groups-list",(req,res)=>res.json({groups:allMatchedGroups.map(n=>({name:n,active:activeGroups.has(n),deleteEnabled:deleteGroups.has(n)}))}));
